...
public class Ribbons : MonoBehaviour
{
    ...
    */
    ...
    private int totalRibbonLength { get { return (int)Mathf.Floor( (float)vertexCount / 8 ); } }
    ...
    private void OnPostRender () {
      ...
      int numVertsTotal = 8 * 3 * 2 * (totalRibbonLength-1);
      ...
      for (int z = 0; z < gridZ; z++) {
        for (int y = 0; y < gridY; y++) {
          for (int x = 0; x < gridX; x++) {
            ...
            float uvX = (float)(id % 8 ) / 8;
            float uvY = Mathf.Floor( (float)(id / 8)) / 4096;
           ...
           // if( uvX == 0 ){ print( uvY ); }
            ...
            float yV = (c + a * Mathf.Cos(v)) * Mathf.Sin(u);
            float zV = a * Mathf.Sin(v);
